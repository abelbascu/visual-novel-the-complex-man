...
public partial class GameStateManager : Node {
    ...
    private void ConfigureStateActions() {
        OnStateChanged += (previousState, previousSubstate, newState, newSubState, arguments) => {

            if (previousSubstate != newSubState) {
                switch (newSubState) {
                    case SubState.LoadScreenInitialized:
                        GameManager.Instance.Initialize_Load_Screen();
                        break;
                    // case SubState.LoadScreenDisplayed:
                    //     GameManager.Instance.Display_Load_Screen();
                    //     break;
                    case SubState.SaveScreenInitialized:
                        GameManager.Instance.Initialize_Save_Screen();
                        break;
                    // case SubState.SaveScreenDisplayed:
                    //     GameManager.Instance.Display_Save_Screen();
                    //     break;
                    case SubState.Loading:
                        if (arguments.Length > 0 && arguments[0] is string filePath) {
                            GameManager.Instance.Load_Game(filePath);
                        } else {
                            GD.PrintErr("Expected file path for loading game, but none was provided.");
                        }
                        break;
                    case SubState.Saving:
                        if (arguments.Length > 0 && arguments[0] is bool isAutosave) {
                            if (isAutosave == false)
                                GameManager.Instance.Save_Game(isAutosave);
                        }
                        // GameManager.Instance.Display_Save_Screen();
                        break;
                    case SubState.AutoSaving:
                        if (arguments.Length > 0 && arguments[0] is bool isAutosave2) {
                            if (isAutosave2 == true)
                                GameManager.Instance.Save_Game(isAutosave2);
                        }
                        // GameManager.Instance.Display_Save_Screen();
                        break;
                              case SubState.LanguageMenuDisplayed:{
                ...
                switch (newState) {
                    ...
                    case State.MainMenuDisplayed:
                        if (previousState == State.MainMenuDisplayed)
                    ...
                    case State.InGameMenuDisplayed:
                        if (previousState == State.InGameMenuDisplayed)
                            GameManager.Instance.Go_Back_To_Menu();
                        else
                    ...
                    case State.EnterYourNameScreenDisplayed:
                        ...
                        break;
                    case State.SettingUpNewGameDialogues:
